...
(defvar yas-keymap  (let ((map (make-sparse-keymap)))
                      (define-key map (kbd "<jd:tab>")   'yas-next-field-or-maybe-expand)
                      (define-key map (kbd "<jd:S-tab>") 'yas-prev-field)
                      (define-key map (kbd "<C-g>")   'yas-abort-snippet)
                      (define-key map (kbd "<jd:bks>")   yas-maybe-clear-field)
                      map)
  "The active keymap while a snippet expansion is in progress.")
...
(defvar yas-minor-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "<jd:tab>") yas-maybe-expand)
    map)
